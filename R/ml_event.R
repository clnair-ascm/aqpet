...
#' @param cdt Logical flag indicating if residual smoothing and decomposition should be done. Default is FALSE.
...
#' )
#'
#' results <- ml_event(df, params, rsd = TRUE, window = 5)
#' }
#'
#' @export
#'
ml_event <- function(df,
                  params,
                  cpd = F,
                  window = 10,
  ...
  data_temp <- df_processed[[length(params$start_time) + 1]]

  response_var <- setdiff(colnames(data_temp), added_time)

  colnames(data_temp)[colnames(data_temp) == response_var] <- "y"
  ...
  mlevent_data <- cbind(df_new, prediction = as.data.frame(h2o.predict(auto_ml_model[[1]], as.h2o(df_new))))
  mlevent_data["difference"] <- mlevent_data[response_var] - mlevent_data["predict"]
  mlevent_data["efficiency"] <- 100 * mlevent_data$difference / mlevent_data["predict"]
